...
class KnittingTimeShow extends React.Component {
  constructor(props){
    super(props);
    this.enrollments = this.props.knitting_time_enrollments.filter( kte => kte.knittingtime_id === parseInt(this.props.ktId))
  }
  ...
  handleSubmit(e){
    e.preventDefault();
    let going_state = this.enrollments.length >= 5 ? false : true

    this.props.me ?
    this.props.createEnrollment({user_id: this.props.meId, knittingtime_id: this.props.ktId, going: going_state}, () => {
                ...
                <div className="ghost">
                  {knittingtime.host_id === this.props.me.id ? <h2>You're the host!</h2> :
                    <div className="mobileinfo">
                      ...
                      {this.enrollments.length >= 5 ? <p>You'll get an email the moment someone cancels their seat.</p> : null}
                      <button className="join" onClick={this.handleSubmit.bind(this)}>{this.enrollments.length >= 5 ? "join waitlist" : "sign me up"}</button>
                    </div>
                  }
                </div>
              }
